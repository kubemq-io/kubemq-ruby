...
module KubeMQBurnin
  module Workers
    class QueuesStreamWorker
      WORKER_NAME = 'queues_stream'
      ...
      def start
        ...
        @sender = KubeMQ::QueuesClient.new(address: @config.broker_address,
                                           client_id: 'burnin-queues-stream-send')
        @receiver = KubeMQ::QueuesClient.new(address: @config.broker_address,
                                             client_id: 'burnin-queues-stream-recv')
      ...
      def cleanup
        generate_channel_names.each do |ch|
          client = KubeMQ::QueuesClient.new(address: @config.broker_address,
                                            client_id: 'burnin-queues-stream-cleanup')
          client.delete_queues_channel(channel_name: ch)
          Metrics.channels_deleted_total.increment(labels: { type: 'queues_stream' })
          client.close
      ...
      def generate_channel_names
        (1..@config.queues_stream_channels).map { |i| "ruby_burnin_#{@run_id}_queues_stream_#{i}" }
      end

      def create_channels(channel_names)
        channel_names.each do |ch|
          @sender.create_queues_channel(channel_name: ch)
          Metrics.channels_created_total.increment(labels: { type: 'queues_stream' })
        rescue StandardError
          # channel may already exist
        end
      end

      # rubocop:disable Metrics/MethodLength -- stream poll receive loop kept linear for clarity
          ...
          thread = Thread.new do
            until @cancel
              begin
                ...
                poll_request = KubeMQ::Queues::QueuePollRequest.new(
                  channel: ch,
                  max_items: @config.poll_max_messages,
                  wait_timeout: @config.poll_wait_timeout,
                  auto_ack: true
                )
                response = @receiver.poll(poll_request)
                elapsed = Process.clock_gettime(Process::CLOCK_MONOTONIC) - start_time

                next if response.nil? || response.error? || response.messages.empty?

                count = response.messages.size
                total_bytes = response.messages.sum { |m| m.body&.bytesize || 0 }
      ...
      def start_senders(channel_names)
        producers_per_ch = @config.queues_stream_producers_per_channel
        rate_per_producer = @config.queues_stream_rate.to_f / producers_per_ch

...
module KubeMQBurnin
  module Workers
    class EventsStoreWorker
      WORKER_NAME = 'events_store'
      ...
      def cleanup
        generate_channel_names.each do |ch|
          client = KubeMQ::PubSubClient.new(address: @config.broker_address,
                                            client_id: 'burnin-events-store-cleanup')
          client.delete_events_store_channel(channel_name: ch)
          Metrics.channels_deleted_total.increment(labels: { type: 'events_store' })
          client.close
      ...
      def generate_channel_names
        (1..@config.events_store_channels).map { |i| "ruby_burnin_#{@run_id}_events_store_#{i}" }
      end

      def create_channels(channel_names)
        channel_names.each do |ch|
          @publisher.create_events_store_channel(channel_name: ch)
          Metrics.channels_created_total.increment(labels: { type: 'events_store' })
        rescue StandardError
      ...
      def start_subscribers(channel_names)
        ...
        channel_names.each do |ch|
          ...
          thread = @subscriber.subscribe_to_events_store(
            sub,
      ...
      def start_publishers(channel_names)
        producers_per_ch = @config.events_store_producers_per_channel
        rate_per_producer = @config.events_store_rate.to_f / producers_per_ch

        channel_names.each do |ch|
          sender = @publisher.create_events_store_sender
          @senders << sender
          ...
          producers_per_ch.times do |_pi|
            thread = Thread.new do
              rate_limited_loop(rate_per_producer) do
                ...
                msg = KubeMQ::PubSub::EventStoreMessage.new(channel: ch, metadata: 'burnin', body: payload)
                sender.publish(msg)
